...
Open [http://localhost:8080/frontend/index.html](http://localhost:8080/frontend/index.html) to see the frontend.
...
Initialize the creationTimestamp to *now*.

Create a service class `ChatMessageService` to load all existing messages for a channel. For now just return a static list. E.g. define this mock data within the same file but outside of the class.
