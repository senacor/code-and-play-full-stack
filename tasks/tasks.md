...
Load messages by channel and ordered by creation time. 
    ...
    List<ChatMessage> findAllByChannelIdOrderByCreationTimestampAsc(String channelId)
...
Extend the `ChatMessageService` with a create method that should take three String parameters (channelId, sender, message)
